...
void playStartupAnimation() {
    ...
    for (int i = 0; i < 50; i++) {
        ...
        canvas.drawString("Anakin v1.1", 160, 118);
        if (i > 10 && i < 40) {
    ...
    while (true) {
        ...
        if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
            if (M5Cardputer.Keyboard.isKeyPressed(';')) { 
                ...
                int ms = 0;
                while (WiFi.status() != WL_CONNECTED && ms < 10000) { 
                    delay(500); 
                    ms += 500; 
                }
                ...
                if (WiFi.status() == WL_CONNECTED) {
                    canvas.fillSprite(BLACK);
                    canvas.setTextColor(GREEN);
                    canvas.drawCenterString("WiFi Connected!", 120, 60);
                    canvas.pushSprite(0, 0);
                    delay(1500);
                    return true;
                } else {
                    ...
                    canvas.drawCenterString("Failed! Try Again.", 120, 60);
                    canvas.pushSprite(0, 0);
                    delay(2000);
                    // 失败后不退出，继续循环扫描界面
                    WiFi.disconnect();
...
void setup() {
    ...
    if (ssid == "" || apiKey == "") {
        ...
        apiKey = getKbdInput("Please Enter API Key");
