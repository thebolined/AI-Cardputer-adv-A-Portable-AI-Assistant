...
void updateDisplay(int inputPos) {
    canvas.fillSprite(BLACK);
    canvas.setFont(&fonts::efontCN_12);
    canvas.fillRoundRect(0, 0, 240, 18, 3, modelColor());
    canvas.setTextColor(WHITE);
        ...
        if (M5Cardputer.Keyboard.isChange() && M5Cardputer.Keyboard.isPressed()) {
            if (M5Cardputer.Keyboard.isKeyPressed(';')) { // 上
                ...
                if (selectedModelIdx == 0) {
                    ...
                    if (kimiApiKey == "") {
                        kimiApiKey = getKbdInput("Enter Kimi API Key");
...
void callAPI() {
    ...
    } else if (selectedModelIdx == 1) { // DeepSeek
        url = "https://api.deepseek.com/chat/completions";
        currentKey = dsApiKey;
    } else { // Kimi (Moonshot AI)
        url = KIMI_API_URL;
        currentKey = kimiApiKey;
    }
    ...
    if (selectedModelIdx == 0) { // Qwen 格式
        ...
        if (selectedModelIdx == 2) {
            ...
            sys["role"] = "system";
            sys["content"] = "You are Kimi, an AI assistant provided by Moonshot AI.";
        }
